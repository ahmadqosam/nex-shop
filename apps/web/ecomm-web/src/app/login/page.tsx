...
export default function LoginPage() {
  const { login, isAuthLoading, authError, clearAuthError } = useAppContext();
    ...
    <div className="min-h-screen flex items-center justify-center bg-secondary/5 px-4 font-sans">
      <div className="bg-background p-8 md:p-16 rounded-[40px] shadow-2xl max-w-lg w-full space-y-12">
        <div className="text-center">
          <h1 className="text-4xl font-bold tracking-tighter mb-4 font-serif text-primary">Welcome Back</h1>
          <p className="text-secondary font-medium">Log in to your Nex account for a personalized sound experience.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <button type="button" className="text-[10px] font-bold underline hover:text-cta transition-colors">Forgot?</button>
            </div>
            <div className="relative">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-sm text-secondary">
            New to Nex? <Link href="/register" className="font-bold text-primary underline hover:text-cta transition-colors">Create account</Link>
          </p>
        </div>
      </div>
    </div>
